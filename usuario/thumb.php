...
$site_config['path_thumbnail']=$site_config['absolute_uri'].'/small/';	//where to cache thumbnails on the server, relative to the DOCUMENT_ROOT
...
$cache_file=str_replace('//','/',$site_config['document_root'].$site_config['path_thumbnail'].str_replace("./full/", "", str_replace(".".$fileextension, "", $filename)).'.'.$fileextension);
/*
...
#padrao e 4000
if ($image_width < 7000)	//no point in resampling images larger than 4000 pixels wide - too much server processing overhead - a resize is more economical
{
